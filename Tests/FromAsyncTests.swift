...
final class FromAsyncTests: XCTestCase {
    func testFromAsync_publishes_value_when_non_throwing_asyncFunction() {
        let exp = expectation(description: "fromAsync publishes the expected value when executing a non throwing asynchronous function")

        let expectedOutput = UUID().uuidString
        var receivedOutput: String?

        // Given: a non throwing async function
    ...
    func testFromAsync_executes_asyncFunction_with_specified_priority_when_non_throwing_asyncFunction() {
        let exp = expectation(description: "fromAsync uses the expected priority when executing a non throwing asynchronous function with a priority")
    ...
    func testFromThrowingAsync_publishes_value_when_throwable_asyncFunction() {
        ...
        // When: making the publisher from the function and subscribing to it
        let cancellable = Publishers
            .fromThrowingAsync { return expectedOutput }
            .handleEvents(receiveCompletion: { _ in exp.fulfill() })
    ...
    func testFromThrowingAsync_executes_asynFunction_with_specified_priority_when_throwing_asyncFunction() {
    ...
    func testFromThrowingAsync_completesWithFailure_when_throwing_asyncFunction() {
        ...
        // When: making the publisher from the function and subscribing to it
        let cancellable = Publishers
            .fromThrowingAsync { throw expectedError }
            .handleEvents(receiveCompletion: { _ in exp.fulfill() })
